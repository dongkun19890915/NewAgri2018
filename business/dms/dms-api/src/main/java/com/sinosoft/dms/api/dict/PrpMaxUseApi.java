package com.sinosoft.dms.api.dict;

import com.sinosoft.dms.api.DMSConstant;
import com.sinosoft.dms.api.dict.dto.PrpMaxUseDto;
import org.springframework.cloud.netflix.feign.FeignClient;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;

import java.util.List;

/**
 * @author codegen@研发中心
 * @mail admin@sinosoft.com.cn
 * @time  2017-11-05 01:02:29.563 
 * @description PrpMaxUseApi接口
 */
@FeignClient(name = DMSConstant.DMS_SERVICE_NAME, path = PrpMaxUseApi.PATH)
public interface PrpMaxUseApi {

    public static final String PATH = "prpMaxUse";

    /**
     *@description 新增
     *@param
     */
    @RequestMapping(value = "save",method = {RequestMethod.POST})
    void save(PrpMaxUseDto prpMaxUseDto);

    /**
     *@description 删除
     *@param
     */
    @RequestMapping(value = "remove",method = {RequestMethod.POST})
    void remove(@RequestParam("groupNo") String groupNo,@RequestParam("tableName") String tableName,@RequestParam("maxNo") String maxNo);
    /**
     *@description 修改
     *@param
     */
    @RequestMapping(value = "modify",method = {RequestMethod.POST})
    void modify(PrpMaxUseDto prpMaxUseDto);
    /**
     *@description 按主键查询实体
     *@param 
     */
    @RequestMapping(value = "queryByPK",method = {RequestMethod.POST})
    PrpMaxUseDto queryByPK(@RequestParam("groupNo") String groupNo,@RequestParam("tableName") String tableName,@RequestParam("maxNo") String maxNo);
}