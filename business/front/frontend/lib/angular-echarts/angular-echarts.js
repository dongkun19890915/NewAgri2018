...
function getLinkFunction($http, theme, util, type) {
    return function (scope, element, attrs) {
        ...
        function getOptions(data, config, type) {
            ...
            if (config.roamController) {
                options.roamController = angular.extend({}, config.roamController);
            }
            if (config.polar) {
    ...
    function getSeries(data, config, type) {
        ...
        angular.forEach(data, function (serie,index) {
            ...
            if (type === 'area' || type==='line') {
                conf.type = 'line';
                conf.itemStyle = config.series[index].itemStyle;
                conf.symbol = config.series[index].symbol;
                conf.symbolSize = config.series[index].symbolSize;
                conf.smooth = config.series[index].smooth;
            ...
            if (isMapChart(type)) {
                ...
                if(angular.isDefined(config.series[index]))
                    conf.itemStyle = config.series[index].itemStyle;
            }
 ...
 */
angular.module('angular-echarts.theme', []).factory('theme', ['infographic', 'macarons', 'shine', 'dark', 'blue', 'green', 'red','catastrophe',
    function (infographic, macarons, shine, dark, blue, green, red, catastrophe) {
    var themes = {
 ...
 * theme
 */
angular.module('angular-echarts.theme').factory('catastrophe', function () {
    return {
        // 默认色板
        color: [
            '#f27318','#0b7cc3'
