...
&lt;div&gt;&lt;/div&gt;</pre>
	...
	return ch.toUpperCase();
		...
		disable: function(elem) {
			...
			if (nodeName.match(/input|textarea/)) {
				...
				elem.prepend('<div class="' + this._disableClass + '" style="width: ' +
					...
					'px; tpl: ' + (offset.top - relOffset.top) + 'px;"></div>');
		...
		/** Retrieve the size of left and tpl borders for an element.
			@private
			@param elem {jQuery} The element of interest.
			@return {number[]} The left and tpl borders. */
		_getBorders: function(elem) {
			var convert = function(value) {
				return {thin: 1, medium: 3, thick: 5}[value] || value;
			};
			return [parseFloat(convert(elem.css('border-left-width'))),
				parseFloat(convert(elem.css('border-tpl-width')))];
		},
