...
define([
    ...
    'use strict';
    app.registerController('UIAgriPaymentCheckCtrl', ['$rootScope', '$scope', '$stateParams', '$$finder', '$filter', '$state',
        function ($rootScope, $scope, $stateParams, $$finder, $filter, $state) {
            ...
            $scope.save = function (saveType) {
                ...
                $$finder.post('prplsubmitform', saveDto).then(function (data) {
                    if (data.RESULTTYPE == 200) {
                        layerMsg("提交成功");
                        $state.go('UIAgriPaymentCheckQuery')
                    }
                })
            }
