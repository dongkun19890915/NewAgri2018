...
;(function(root, factory) {
    ...
    jedfn.initdates = function(){
        ...
        var formatDate = function (cls,boxcell,target) {
            ...
            // ****为了支持手动输入 做以下改动
            //源码： var vals = that.getValue({});
            var vals = that.getValue({},target.attr("valid_date"));
            //****改动结束
            $(cls).append(dateDiv);
        ...
        if(!isShow){
            ...
            if (jetrigger) {
                that.valCell.on(jetrigger, function (ev) {
                    ...
                    formatDate(jd[0],jd[1],$(ev.target));
                });
            }else {
    ...
    //获取日期值
    jedfn.getValue = function (valobj,getCurDate) {
