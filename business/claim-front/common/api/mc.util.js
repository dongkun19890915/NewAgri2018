...
define([
    ...
    //layer弹框提示
    Window.prototype.layerMsg = function(data,callBack,conf){
        // var icon = target == 'success'?"1":target == 'error'?"2":"0";
        var htm = '<div style="font-size: 14px;line-height: 24px;min-width: 260px;max-height: 160px">'
        if(conf && conf.skin){
            htm += '<i class="iconfont layer-skin-font ';
            htm += conf.skin  ;
            htm += '"></i>';
        }
        htm +=(data+'</div>');
        var index = layer.alert( htm,{
            title: '提示',
            closeBtn: 1,
            shadeClose: true,
            skin: 'layui-layer-ico1',
            cancel:callBack
        },function () {
