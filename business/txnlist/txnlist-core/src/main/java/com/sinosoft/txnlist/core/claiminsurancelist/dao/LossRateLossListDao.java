 ...
 * 定损清单农户标的清单表Dao数据操作对象
 */
@Repository
public interface LossRateLossListDao extends JpaBaseDao<LossRateLossList,LossRateLossListKey> {
    /**
     * 根据清单号和序列号查询清单信息集合
     ...
     */
    @Transactional
    @Query(value = "select l from LossRateLossList l where l.lossListCode=:lossListCode and l.serialNo=:serialNo")
    List<LossRateLossList> queryBylossListCodeAndSerialNo(@Param(value="lossListCode")String lossListCode,
                                                          @Param(value="serialNo")Integer serialNo);

    @Transactional
    @Modifying
    @Query(value = "delete from LossRateLossList l where l.lossListCode in :lossListCode and l.serialNo=:serialNo ")
