...
var residency = {
    ...
    Show : function(){
        ...
        // 点击弹出子菜单
        $('#allProv li').click(function(e){$("#sublist").css({top:$(window).height()/3,left:$(window).width()/3 }).hover(function(){$(this).show()},function(){$(this).hide()})})
    },
    // 所有省份 下拉 城市菜单
    SubLayer : function(id){
        ...
        for (var i=1;i<arr.length;i++){
            key=arr[i];
            if(i==1){
                output+='<li><a href="javascript:;" onclick="residency.Chk(\''+key+'\')"><font color="red">'+ja[key]+'</font></a></li>';
            }else{
    ...
    Chk : function(id){
        ...
        $('#relationListQueryLayer').hide();
        $("#sublist").hide().empty();
        boxAlpha();
    }
};

function closeRelationListLayer(){
    $('#relationListQueryLayer').show();
...
function hukouSelect(){
    ...
    dragHtml+='</div>';
    $('#drag_h').html('<b>收款人开户银行所在地</b><span onclick="boxAlpha()">关闭</span>');
