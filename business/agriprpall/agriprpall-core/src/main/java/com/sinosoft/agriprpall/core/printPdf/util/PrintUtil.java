package com.sinosoft.agriprpall.core.printPdf.util;
...
public class PrintUtil {
     ...
     * @date 2017/9/3 16:44
     */
    public static void printReportPdfMore(List<Map<String,Object>> listMap, int n, OutputStream outputStream) throws Exception {
     ...
     * @date 2017/9/3 16:44
     */
    public static void printReportPdfSingle(Map<String,Object> map, int n, OutputStream outputStream) throws Exception {
        ArrayList<JasperPrint> jasperPrintList = new ArrayList<JasperPrint>();
        InputStream inputStream;
        for(int j =0; j<n; j++){
                inputStream = (InputStream) map.get("templetPath");
                ArrayList list;
                ...
                JRDataSource jrDataSource = new JRBeanCollectionDataSource(list);
                JasperReport jasperReport = JasperCompileManager.compileReport(inputStream);
                jasperPrintList.add(JasperFillManager.fillReport(jasperReport, map, jrDataSource));
        }
        JRPdfExporter exporter = new JRPdfExporter();
        Map<FontKey,PdfFont> fontsMap = new HashMap<FontKey,PdfFont>();
        //此处用黑体代表STSong-Light在pdf中的粗体，模板中选择的字体需要是黑体
