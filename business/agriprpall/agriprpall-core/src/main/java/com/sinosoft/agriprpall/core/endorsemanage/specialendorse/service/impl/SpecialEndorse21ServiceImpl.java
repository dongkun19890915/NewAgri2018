...
import com.sinosoft.agriprpall.core.endorsemanage.specialendorse.service.SpecialEndorse21Service;
...
@Service
public class SpecialEndorse21ServiceImpl extends BaseServiceImpl implements SpecialEndorse21Service{
    @Autowired
    ...
    public PolicyEndorseDto specialEndorse21(BLEndorseDto blEndorseDto, EndorseConditionDto endorseConditionDto) throws Exception {
        ...
        /**1、置Prp*main.OthFlag**/
        //全单退保（21）、犹豫期退保（62）：OthFlag[3]=1
        strOthFlag = prpCmainDto.getOthFlag();
        if (strOthFlag.length() >= 3) {
            strOthFlag = strOthFlag.substring(0, 2) + "1" + strOthFlag.substring(3);
        } else {
            strOthFlag = strOthFlag + Str.space(2 - strOthFlag.length()) + "1";
        }
        ...
        if (StringUtils.isNotEmpty(breedingFarmerListFlag) && StringUtils.isNotEmpty(strRiskCode) && breedingFarmerListFlag.indexOf(strRiskCode) > -1 && !"3224".equals(strRiskCode)) {
            if (insureMainListDtoList.size() > 0) {
                ...
                //全单退保（21）
                strKindCode = prpCitemKindDtoList.get(0).getKindCode();
                //计算短期系数
                intStartHour = prpCitemKindDtoList.get(0).getStartHour();
                endDate = strValidDate;
                intEndHour = strValidHour;
                finishDate = prpCitemKindDtoList.get(0).getEndDate();
                intFinishHour = prpCitemKindDtoList.get(0).getEndHour();
                for (int j = 0; j < blPolicy.getHerdPolicyListDtoList().size(); j++) {
                    if ("1".equals(blPolicy.getHerdPolicyListDtoList().get(j).getValidity())) {
                        startDate = blPolicy.getHerdPolicyListDtoList().get(j).getStartDate();
                        //直接计算已了责任区间对应的短期系数（短期系数的目的：退费要少、收费要多）
                        //国元需求，统一公式计算
                        dblShortRate = pubTools.getCommerceShortRate(strRiskCode,
                                ...
                                new com.sinosoft.utility.string.Date(sdf.format(finishDate)), intFinishHour,
                                strShortRateFlag);
                        //计算保费
                        //批改后保费=原保费*新短期系数/原短期系数
                        dblShortRateOld = herdPolicyListDtoList.get(j).getShortRate();
                        dblPremiumOld = Str.round(herdPolicyListDtoList.get(j).getSumPremium(), 2);

                        dblPremiumNew = Str.round(dblPremiumOld * dblShortRate / dblShortRateOld, 2);
                        PolicySumPremium = PolicySumPremium + dblPremiumNew;//总保费
                        //终保日期
                        blPolicy.getHerdPolicyListDtoList().get(j).setEndDate(strValidDate);
                        //短期费率方式
                        blPolicy.getHerdPolicyListDtoList().get(j).setShortRateFlag(strShortRateFlag);
                        //短期系数
                        blPolicy.getHerdPolicyListDtoList().get(j).setShortRate(dblShortRate);
                        //保费
                        blPolicy.getHerdPolicyListDtoList().get(j).setSumPremium(dblPremiumNew);
                        //农户自缴保费
                        blPolicy.getHerdPolicyListDtoList().get(j).setInsurePremium(Str.round(dblPremiumNew * selfRate, 2));
                        //保额置为0
                        blPolicy.getHerdPolicyListDtoList().get(j).setSumAmount(0.00);
                        blPolicy.getHerdPolicyListDtoList().get(j).setValidity("0");
                        blPolicy.getHerdPolicyListDtoList().get(j).setFlag("B");
                    }
                }
                prpCitemKindDtoList.get(0).setPremium(PolicySumPremium);
        ...
        for (index = 0; index < prpCitemKindDtoList.size(); index++) {
            ...
            String strKey = prpCitemKindDtoList.get(index).getKindCode() + "-"
            ...
            for(int i=0;i<blPolicy.getPrpCitemKindAgriDtoList().size();i++){
                if (prpCitemKindAgriKey.equals(blPolicy.getPrpCitemKindAgriDtoList().get(i).getItemKindNo()+"-"+blPolicy.getPrpCitemKindAgriDtoList().get(i).getKindCode())) {
                    blPolicy.getPrpCitemKindAgriDtoList().get(i).setUnitAmount(0.00);
                }
            }
            ...
            /* begin reason：调整全单退保 */
            //货运险全单退保（21）
            if (strEndorType.equals(AgriPrpallConstant.EDITTYPE_WITHDRAW.trim()) && strShortRateFlag.equals("")) {
                //保额置为0
            ...
            //计算短期系数
            startDate = prpCitemKindDtoList.get(index).getStartDate();
            intStartHour = prpCitemKindDtoList.get(index).getStartHour();
            endDate = strValidDate;
            intEndHour = strValidHour;
            finishDate = prpCitemKindDtoList.get(index).getEndDate();
            intFinishHour = prpCitemKindDtoList.get(index).getEndHour();
            //直接计算已了责任区间对应的短期系数（短期系数的目的：退费要少、收费要多）
            //国元需求，统一公式计算
            //todo
            dblShortRate = pubTools.getCommerceShortRate(strRiskCode,
            ...
            dblPremiumOld = Str.round(prpCitemKindDtoList.get(index).getPremium(), 2);

            if ("3221".equals(strRiskCode) || "3229".equals(strRiskCode)) {
                if (dblShortRateOld == 0) {
                    dblPremiumNew = 0.00;
                } else {
                    dblPremiumNew = Str.round(dblPremiumOld * dblShortRate / dblShortRateOld + dblPremiumOld * (1 - dblShortRate / dblShortRateOld) * (Double.parseDouble(ChgData.chgStrZero(strIrate)) / 100), 4);
                }
            }
            //国元需求，统一公式计算
            else {
                if (dblShortRateOld == 0) {
                    ...
                    dblPremiumNew = Str.round(dblPremiumOld * dblShortRate / dblShortRateOld, 4);
                }
            }
            if (StringUtils.isNotEmpty(plantingFarmerListFlag) && StringUtils.isNotEmpty(strRiskCode) && plantingFarmerListFlag.indexOf(strRiskCode) > -1) {
                if (insureMainListDtoList.size() > 0) {
                    for(int i=0;i<blPolicy.getPrpCitemKindDtoList().size();i++){
                        for(int j=0;j<plantingPolicyListDtoList.size();j++){
                            if("1".equals(plantingPolicyListDtoList.get(j).getValidity()) &&
                             blPolicy.getPrpCitemKindDtoList().get(i).getKindCode().equals(plantingPolicyListDtoList.get(j).getKindCode())){
                                startDate = blPolicy.getPrpCitemKindDtoList().get(i).getStartDate();
                                //直接计算已了责任区间对应的短期系数（短期系数的目的：退费要少、收费要多）
                                //国元需求，统一公式计算
                                dblShortRate = pubTools.getCommerceShortRate(strRiskCode,
                                        new com.sinosoft.utility.string.Date(sdf.format(startDate)), intStartHour,
                                ...
                                //计算保费
                                //批改后保费=原保费*新短期系数/原短期系数
                                //dblShortRateOld = blPolicy.getPrpCitemKindDtoList().get(i).getShortRate().doubleValue();
                                //dblPremiumOld = Str.round(blPolicy.getPrpCitemKindDtoList().get(i).getSumPremium().doubleValue(),2);
                                //dblPremiumNew = Str.round(dblPremiumOld*dblShortRate/dblShortRateOld,2);
                                //终保日期
                                plantingPolicyListDtoList.get(i).setEndDate(strValidDate);
                                //短期费率方式
                                plantingPolicyListDtoList.get(i).setShortRateFlag(strShortRateFlag);
                                //短期系数
                                plantingPolicyListDtoList.get(i).setShortRate(dblShortRate);
                                //保额置为0
                                plantingPolicyListDtoList.get(i).setSumAmount(0.00);
                                plantingPolicyListDtoList.get(i).setCentralPremium(0.00);
                                plantingPolicyListDtoList.get(i).setProvincePremium(0.00);
                                plantingPolicyListDtoList.get(i).setCityPremium(0.00);
                                plantingPolicyListDtoList.get(i).setTownPremium(0.00);
                                plantingPolicyListDtoList.get(i).setOtherPremium(0.00);
                                plantingPolicyListDtoList.get(i).setfPremium(0.00);
                                plantingPolicyListDtoList.get(i).setValidity("0");
                                plantingPolicyListDtoList.get(i).setFlag("B");
                            }
                        }
                    }
                    dblPremiumNew = 0.00;
            ...
            if (("3141".equals(strRiskCode)) || ("3147".equals(strRiskCode))) {
                if (insureMainListDtoList.size() > 0) {
                    for (int i = 0; i < planting31PolicyListDtoList.size(); i++) {
                        if ("1".equals(planting31PolicyListDtoList.get(i).getValidity())) {
                            if (strKindCode.equals(planting31PolicyListDtoList.get(i).getKindCode())) {
                                startDate = planting31PolicyListDtoList.get(i).getStartDate();
                                //直接计算已了责任区间对应的短期系数（短期系数的目的：退费要少、收费要多）
                                //国元需求，统一公式计算
                                dblShortRate = pubTools.getCommerceShortRate(strRiskCode,
                                        new com.sinosoft.utility.string.Date(sdf.format(startDate)), intStartHour,
                                ...
                                //计算保费
                                //批改后保费=原保费*新短期系数/原短期系数
                                //dblShortRateOld = prpCitemKindDtoList.get(i).getShortRate().doubleValue();
                                //dblPremiumOld = Str.round(prpCitemKindDtoList.get(i).getSumPremium().doubleValue(),2);
                                //dblPremiumNew = Str.round(dblPremiumOld*dblShortRate/dblShortRateOld,2);
                                //终保日期
                                planting31PolicyListDtoList.get(i).setEndDate(strValidDate);
                                //短期费率方式
                                planting31PolicyListDtoList.get(i).setShortRateFlag(strShortRateFlag);
                                //短期系数
                                planting31PolicyListDtoList.get(i).setShortRate(dblShortRate);
                                //保额置为0
                                planting31PolicyListDtoList.get(i).setSumAmount(0.00);
                                planting31PolicyListDtoList.get(i).setCentralPremium(0.00);
                                planting31PolicyListDtoList.get(i).setProvincePremium(0.00);
                                planting31PolicyListDtoList.get(i).setCityPremium(0.00);
                                planting31PolicyListDtoList.get(i).setTownPremium(0.00);
                                planting31PolicyListDtoList.get(i).setOtherPremium(0.00);
                                planting31PolicyListDtoList.get(i).setfPremium(0.00);
                                planting31PolicyListDtoList.get(i).setValidity("0");
                    ...
                    for (int i = 0; i < nyxPolicyListDtoList.size(); i++) {
                        if ("1".equals(nyxPolicyListDtoList.get(i).getValidity())) {
                            if (strKindCode.equals(nyxPolicyListDtoList.get(i).getKindCode())) {
                                startDate = nyxPolicyListDtoList.get(i).getStartDate();
                                //直接计算已了责任区间对应的短期系数（短期系数的目的：退费要少、收费要多）
                                //国元需求，统一公式计算
                                dblShortRate = pubTools.getCommerceShortRate(strRiskCode,
                                        new com.sinosoft.utility.string.Date(sdf.format(startDate)), intStartHour,
                                ...
                                //dblPremiumNew = Str.round(dblPremiumOld*dblShortRate/dblShortRateOld,2);
                                //终保日期
                                nyxPolicyListDtoList.get(i).setEndDate(strValidDate);
                                //短期费率方式
                                ...
                                nyxPolicyListDtoList.get(i).setFlag("B");
                                //planting31PolicyListDtoList.get(i).setFlag("B");
                            }
                        }
                    }
            ...
            // 农险二期平台改造 新加清单的险种  全单退保 处理policy表保费
            if (nyxSingleFarmerListFlag.contains(strRiskCode) || nyxMultipleFarmerListFlag.contains(strRiskCode)) {
                if (insureMainListDtoList.size() > 0) {
                    for (int i = 0; i < nyxPolicyListDtoList.size(); i++) {
                        if ("1".equals(nyxPolicyListDtoList.get(i).getValidity())) {
            ...
            //终保日期
            prpCitemKindDtoList.get(index).setEndDate(strValidDate);
            prpCitemKindDtoList.get(index).setEndHour(strValidHour);
            //短期费率方式
            ...
            //保额置为0
            prpCitemKindDtoList.get(index).setAmount(0.00);
            //prpCitemKindDtoList.get(index).setUnitAmount(0.00);//老系统没有置为0，新系统置为0，因为批单详情页面如果有值就会自动进行计算。
            //prpCitemKindDtoList.get(index).setQuantity(0.00);
            //保费
            if (null != breedingFarmerListFlag && breedingFarmerListFlag.indexOf(strRiskCode) > -1 && insureMainListDtoList.size() > 0 && !"3224".equals(strRiskCode)) {
            } else {
            ...
            if (strEndorType.equals(AgriPrpallConstant.EDITTYPE_WRITEOFF.trim()) ||
                ...
                /* 农险统计中用到批单的承保数量和参保农户户次两个字段，因此全单退保要对这两个字段清零 */
                prpCmainDto.setSumInsured(0.00);
                prpCmainDto.setStatQuantity(0.00);
            }
        }
        ...
        //全单退保（21）
        //终保日期
        prpCmainDto.setEndDate(strValidDate);
        prpCmainDto.setEndHour(strValidHour);
        //总保额（保单汇总币别）
        ...
        prpCmainDto.setRemark(blPolicyOld.getPrpCmainDto().getRemark());
        ...
        //保单注销、全单退保、手续费批改、特殊因子批改
        if (strEndorType.equals(AgriPrpallConstant.EDITTYPE_WRITEOFF.trim())
                || strEndorType.equals(AgriPrpallConstant.EDITTYPE_WITHDRAW.trim())
                || strEndorType.indexOf(AgriPrpallConstant.EDITTYPE_COMMISSION.trim()) > -1
                || strEndorType.equals(AgriPrpallConstant.EDITTYPE_MIDDLECOST.trim())) {
...
//                int i = 0;
//                Planting31EndorChgDetailDto planting31EndorChgDetailSchema = null;
//                Planting31PolicyListDto planting31PolicyListOldSchema = null;
//                Planting31PolicyListDto planting31PolicyListNewSchema = null;
//                Planting31PolicyListDto planting31PolicyListNewSchemaTmp = null;
//                Hashtable hashItemKind = new Hashtable();
//                List<Planting31EndorChgDetailDto> planting31EndorChgDetailDtoList = new ArrayList<>();
//                //新的保单中的对象个数应该大于等于旧的保单中的对象个数（因为删除的对象依旧记录在新的保单中）
//                if (blPolicy.getPlanting31PolicyListDtoList().size() < PPolicyOld.getPlanting31PolicyListDtoList().size()) {
//                } else {
//                    //将indexCode与其Schema建立映射关系
//                    for (i = 0; i < PPolicyOld.getPlanting31PolicyListDtoList().size(); i++) {
//                        planting31PolicyListNewSchemaTmp = new Planting31PolicyListDto();
//                        planting31PolicyListNewSchemaTmp = PPolicyOld.getPlanting31PolicyListDtoList().get(i);
//                        hashItemKind.put("" + planting31PolicyListNewSchemaTmp.getfIdCard() + planting31PolicyListNewSchemaTmp.getKindCode() + planting31PolicyListNewSchemaTmp.getItemCode(), planting31PolicyListNewSchemaTmp);
//                    }
//                    //建立映射关系完毕
//                    for (i = 0; i < blPolicy.getPlanting31PolicyListDtoList().size(); i++) {
//                        planting31PolicyListNewSchema = blPolicy.getPlanting31PolicyListDtoList().get(i);
//                        planting31PolicyListOldSchema = new Planting31PolicyListDto();
//                        if (planting31PolicyListNewSchema.getFlag() == null) {
//                            planting31PolicyListNewSchema.setFlag("");
//                        }
//                        if (planting31PolicyListNewSchema.getFlag().length() > 0) {
//                            if (planting31PolicyListNewSchema.getFlag().charAt(0) == 'I') {
//                                planting31PolicyListOldSchema = planting31PolicyListNewSchema;
//                            } else {
//                                planting31PolicyListOldSchema = (Planting31PolicyListDto) hashItemKind.get(""
//                                        + planting31PolicyListNewSchema.getfIdCard() + planting31PolicyListNewSchema.getKindCode() + planting31PolicyListNewSchema.getItemCode());
//                                if (planting31PolicyListOldSchema == null) {
//                                    continue;
//                                }
//                            }
//                            planting31EndorChgDetailSchema = new Planting31EndorChgDetailDto();
//                            planting31EndorChgDetailSchema.setEndorseNo(blEndorseDto.getPrpPheadDto().getEndorseNo());
//
//                            planting31EndorChgDetailService.setPlanting31EndorChgDetail(planting31EndorChgDetailSchema, planting31PolicyListOldSchema, planting31PolicyListNewSchema);
//                            planting31EndorChgDetailDtoList.add(planting31EndorChgDetailSchema);
//                        }
//                    }
//                }
//                blEndorseDto.setPlanting31EndorChgDetailDtoList(planting31EndorChgDetailDtoList);
        ...
        //blEndorse.webAfterCal(blPolicy);//plan表等调差 农险没有此操作
